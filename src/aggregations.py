...
def volume_exames_por_zip(transactional: pd.DataFrame, geocode: pd.DataFrame) -> pd.Series:
...
def custo_medio_por_zip(transactional: pd.DataFrame, geocode: pd.DataFrame) -> pd.Series:
    merged = transactional.merge(geocode[['Lab Id', 'Zipcode']], on='Lab Id', how='left')
    return merged.groupby('Zipcode')['Testing Cost'].mean().sort_values(ascending=False)

def volume_exames_por_faixa_etaria(transactional: pd.DataFrame) -> pd.Series:
...
def top_coditem(transactional: pd.DataFrame, top_n: int = 10) -> pd.DataFrame:
